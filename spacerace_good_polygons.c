...
static void cube() {
   ...
   static const char filename[] = "goddamncube.obj";
   ...
   if (file != NULL) {
      ...
      while (fgets (line, sizeof line, file) != NULL) {
         ...
         if (line[0] == 'f' && line[1] == ' ') {
            ...
            while(p) {
               printf("trying to split the line\n");
               res = realloc(res, sizeof (char*) * ++n_spaces);
               if (res == NULL) 
                  exit(-1);
               res[n_spaces-1] = p;
            ...
            int i;
            for (i = 1; i <= 3; i++) {
               printf("trying to split the individual coordinates\n");
               char ** res2 = NULL;
               char * p2 = strtok(res[i], "/");
               int n_spaces2 = 0;
...
void display() {
   ...
   draw_obj(326, 365, 365, 646, "voyagertri.obj");
